...
const loggin: Router = new Router();
...
loggin.post('/', registrationValidation, async (ctx: Context) => {
    ...
    // await newUser.save();
     ...
     const token = generateToken( user.username , user.password , 'user') // must be from the database
    ...
    ctx.status = 201;
    ctx.body = creatingSuccessBody('User loggined successfully' , response );
